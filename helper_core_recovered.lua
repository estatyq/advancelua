    ...
    sampRegisterChatCommand("healme", function()
        lua_thread.create(function()
            if modules[3].enabled and rp_heal_enabled[0] then
                sampSendChat(u8:decode("/me opened a medical kit, took a bandage and bandaged the wound"))
                    ...
                    if title_utf8:find("Редактирование") or text_utf8:find("Текст объявления:") then
                        ...
                        if original_ad and original_ad ~= "" then
                            ...
                            if mm_auto_send[0] then
                                ...
                                -- Закрываем диалог на стороне клиента, чтобы игрок его не видел
...
function applyLocalSkin(skinId)
    lua_thread.create(function()
        if isValidModel(skinId) then
            ...
            sampAddChatMessage(u8:decode("[Helper] Некорректный ID скина."), 0xFF0000)
        end
    end)
end
    ...
    while strobe_active and strobe_enabled[0] do
        if isCharInAnyCar(PLAYER_PED) then
            ...
            if car and doesVehicleExist(car) then
                ...
                state = not state
                if strobe_mode[0] == 0 then
                    if state then
                        setCarLightDamageStatus(car, 0, 0)
                        setCarLightDamageStatus(car, 1, 0)
                    else
                        setCarLightDamageStatus(car, 0, 1)
                        setCarLightDamageStatus(car, 1, 1)
                    end
                else
                    if state then
                        setCarLightDamageStatus(car, 0, 0)
...
function cruiseControlWorker()
    while true do
        ...
        if modules[4].enabled and cruise_enabled[0] then
            if isCharInAnyCar(PLAYER_PED) then
                local car = storeCarCharIsInNoSave(PLAYER_PED)
                if isKeyDown(0x43) and not isSampDiagActive() and not sampIsChatInputActive() then -- C
...
function weaponTrackWorker()
    ...
    while true do
        ...
        if modules[3].enabled and rp_weapons_enabled[0] then
            if not isSampDiagActive() and not sampIsChatInputActive() then
...
imgui.OnFrame(
    ...
    function(player)
        ...
        imgui.SameLine()
        imgui.PushItemWidth(150)
        if imgui.Combo("##ServerSelector", current_server_idx, server_names, #server_names) then
            saveSettings()
            sampAddChatMessage(u8:decode("[Helper] Сервер изменен на: " .. u8:encode(ffi.string(server_names[current_server_idx[0]]), encoding.default)), 0x00FF90)
        end
        imgui.PopItemWidth()
