...
## Шаг 3. Установка MoonLoader (со всеми библиотеками)

MoonLoader — это загрузчик Lua-скриптов для GTA SA.

1. Скачайте **MoonLoader v0.26+ со всеми библиотеками** (полная сборка):
   - https://libertycity.net/files/gta-san-andreas/131091-moonloader-0.27.html
   - Или с официальной темы: https://gtaforums.com/topic/890987-moonloader/
   > **Важно:** качайте **полную сборку** (со всеми библиотеками), а не минимальную.
   > Полная сборка уже содержит: `mimgui`, `SAMP.Lua` (samp.events), `memory`, `bit`, `encoding`, `json`, `ffi`.
   > Если скачать минимальную версию — библиотеки придётся ставить вручную (см. Шаг 5).
...
3. После установки появится папка `moonloader/` со структурой:
   ...
   └── moonloader/
       ...
       ├── lib/               (библиотеки Lua: mimgui, samp/, ...)
...
## Шаг 5. Установка библиотек (если не полная сборка)

> **Если вы скачали полную сборку MoonLoader (Шаг 3) — пропустите этот шаг.**
> Все нужные библиотеки уже установлены.
>
> Этот шаг нужен только если вы скачали минимальную версию MoonLoader.
